...
import csv
''' Imports a plain-text file that functions as a spreadsheet
for storing data.  CSV is an ancronym for Comma Separated Values.  This module
will store the data as a separate file that can be updated and accessed
whenever needed.
'''
...
def get_and_check_ans(cor_ans):
    ...
    if ans == cor_ans:
        ...
        if ans == cor_ans:
            ...
            final_score = final_score + 5                             #second attempt
        else:
...
def run():
    ...
    print('Your final final_score is', final_score, 'points out of a possible 110!!')
...
with open ("protleader.csv", "a", newline='') as file:
    fields=['final_score', 'name']
    writer=csv.DictWriter(file, fieldnames=fields)
    writer.writerow({'final_score' : final_score, 'name' : username})
